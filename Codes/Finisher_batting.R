...
player_dataset<-read.csv("E:/Cricinfo/cricinfo_batting.csv",stringsAsFactors = FALSE)

# Reading match results data set

match_result_dataset<-read.csv("E:/Cricinfo/cricinfo_match_results.csv",stringsAsFactors = FALSE)
...
# Inns = first or second innings in a ODI
...
# Applying the finisher criteria
final<-data.frame()
...
for(i in 1:length(unique(final_dataset$Player.Name)))
{
     ...
     # Criteria Runs>=30, Margin = 6, BR = 30 (5 overs), Carried.Status = Not-out, Result = won, 2nd batting
...
write.csv(final,"E:/Cricinfo/finisher_score_denom_change.csv",row.names = FALSE)

# write.csv(corres_player,"E:/Cricinfo/player.csv",row.names = FALSE)
